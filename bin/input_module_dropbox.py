...
def validate_input(helper, definition):
    ...
    access_token = definition.parameters.get('access_token')
    start_time = definition.parameters.get('start_time')
    category = definition.parameters.get('category')

    is_access_token_encrypted = True
    if is_access_token_encrypted:
        # as for now we can't read encrypted fields, we just check time format
        get_start_time(start_time)
    else:
        # check all params by performaning call to get events
        response = send_http_request(helper, None, access_token, start_time, category)
